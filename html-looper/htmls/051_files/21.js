...
(function() {
  'use strict';
  var Follower, Square, animloop, canvas, ctx, hsla, i, rotateTo, spread, squares, tail, tilt, timeInc, _i;
  ...
  /*
    ...
    Mouse click starts new path
    Mouse move adds points to this new path
  */
  ...
  Follower = (function() {
    ...
    Follower.prototype.draw = function() {
      var hue, light, rotation, size;

      hue = (this.index / tail * 60 - timeInc(64)) % 360;
      light = this.index / tail * 40 + 8;
      size = 1 + this.index * 6;
      ctx.save();
      ...
      rotation = Math.sin(this.index / (Math.cos(timeInc(32000)) + 2) + this.clickRotate / 64);
      ctx.rotate(rotation);
      ctx.fillStyle = hsla(hue, 100, light, 1 - this.index / tail);
  ...
  animloop = function() {
    ...
    ctx.decay(3 * -followers[Math.round(followers.length / 2)].x0 / canvas.width, 3 * -followers[Math.round(followers.length / 2)].y0 / canvas.height, 4, tilt(180, followers[Math.round(followers.length / 2)].x0, followers[Math.round(followers.length / 2)].y0));
