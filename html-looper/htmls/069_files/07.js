...
(function() {
  var CLICKED_DURATION, Configuration, FRAME, Follower, NOW, PI, TIME, abs, animloop, between, clear, cos, decay, degToRad, hsla, hyp, imageSmoothing, initGui, initWave, radToDeg, rainbow, rainbowCircle, rgba, sin, tan, tick;
  ...
  Follower = (function() {
    ...
    Follower.prototype.follow = function() {
      ...
      return this.y = canvas.bottom + waves;
    };

    Follower.prototype.draw = function() {
      var hue;
      hue = rainbow(TIME / 10 + this.x * cfg.hue_w + this.y * cfg.hue_h, 1 * cfg.sat);
      ctx.fillStyle = hue;
      return ctx.fillRect(this.x | 0, (this.y | 0) - 60, 0.5, 0.5);
    };
  ...
  initWave = function() {
    ...
    _results = [];
    for (i = _i = _ref = canvas.left, _ref1 = canvas.right; _i <= _ref1; i = _i += 1) {
      _results.push(crowd[i] = new Follower(i, Mouse.x, Mouse.y));
    }
    return _results;
  };
  ...
  animloop = function() {
    ...
    tick();
    for (i = _i = _ref = canvas.left, _ref1 = crowd.length; _i < _ref1; i = _i += 1) {
      crowd[i].follow();
      crowd[i].draw();
    }
    return decay(ctx, 0, -1, 0, 0, 0);
  };
