...
(function() {
  ...
  Square = (function() {
    ...
    Square.prototype.draw = function() {
      var height, hue, hueHyp, i, light, mouseAdj, rise, rotation, run, size, sizeHyp, squareRotation, width, x, y;

      i = this.index;
      size = Math.hyp(canvas.height, canvas.width) / 18;
      sizeHyp = Math.hyp(this.x / 300, this.y / 300);
      height = size * Math.sin(sizeHyp - timeInc(6400));
      width = height;
      x = -width / 2;
      y = -height / 2;
      hueHyp = Math.hyp(this.x / 10, this.y / 10);
      rise = this.x + canvas.right * 1.2;
      run = this.y + canvas.top * 1.2;
      rotation = Math.radToDeg(rotateTo(rise, run));
      mouseAdj = Math.hyp(Mouse.xs / 4 - this.x / 4, Mouse.ys / 4 - this.y / 4);
      hue = hueHyp + rotation + mouseAdj - timeInc(48);
      light = (Math.sin(Math.degToRad(hue) - timeInc(20000)) + 1) * 50;
      squareRotation = Math.degToRad(i / 3 - timeInc(60) % 360);
      ctx.save();
