...
(function() {
  'use strict';
  var Mouse, Square, animloop, canvas, ctx, decay, hsla, rot, squares;
  ...
  Square = (function() {
    ...
    Square.prototype.draw = function() {
      var h, hue, hypotenuse, i, light, rise, rotation, run, size, slope, tangent, w, x, y;

      i = this.index;
      rise = this.x - Mouse.x;
      run = this.y - Mouse.y;
      slope = rise / run;
      tangent = Math.atan(slope) / (Math.PI / 180);
      if (rise > 0 || run > 0) {
        rotation = -(tangent + 180) * Math.PI / 180;
      } else {
        rotation = -(tangent * Math.PI / 180);
      }
      hypotenuse = Math.floor(Math.hyp(rise, run));
      size = Math.floor(canvas.max / 18);
      h = Math.min(size, hypotenuse);
      w = h;
      x = -w / 2;
      y = -h / 2;
      ctx.save();
      ctx.translate(Math.floor(this.x), Math.floor(this.y));
      ctx.rotate(rotation + (-Date.now() / 36 * Math.PI / 180) % 360);
      hue = (-Date.now() / 77 + hypotenuse) % 360;
      light = (Math.sin(hypotenuse / 90 - Date.now() / 1080) + 1) * 50;
