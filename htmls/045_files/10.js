...
(function() {
  'use strict';
  var Mouse, Square, animloop, canvas, ctx, hsla, rotateTo, squares, tilt, timeInc;
  ...
  Square = (function() {
    ...
    Square.prototype.draw = function() {
      ...
      h = Math.round(size * Math.sin(timeInc(6400) + sizeHyp));
      w = Math.round(h);
      x = Math.round(-w / 2);
      y = Math.round(-h / 2);
      ctx.save();
      ctx.translate(Math.floor(this.x), Math.floor(this.y));
      rotation = Math.radToDeg(rotateTo(this.x + canvas.right * 1.2, this.y + canvas.top * 1.2));
      mouseAdj = Math.hyp(Mouse.xs / 4 - this.x / 4, Mouse.ys / 4 - this.y / 4);
      hue = Math.round(hypotenuse + rotation + mouseAdj + timeInc(48));
      light = Math.round((Math.sin(Math.degToRad(hue) + timeInc(20000)) + 1) * 50);
      squareRotation = Math.degToRad(timeInc(60) % 360 + i / 3);
      ctx.rotate(Math.round(squareRotation * 1000) / 1000);
  ...
  animloop = function() {
    ...
    if (Mouse.clicks % 2 === 0) {
      ctx.decay(-Mouse.xs / canvas.width * 1, -Mouse.ys / canvas.width * 1, 2.5, -tilt(120));
    }
    return squares.draw();
  };
